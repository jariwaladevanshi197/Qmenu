...
if (isset($_SERVER['HTTP_REFERER'])) {
    ...
    $query = "select * from orderhistory where restroid = '$restroid' AND DATE(timestamp) = CURDATE() $limit";
    ...
    $query = "select SUM(grandtotal) AS grandtotal,count(id) AS totalorder from orderhistory where restroid = '$restroid'";
    $res = mysqli_query($con, $query);
    ...
    if ($last_page > 1) {
        if ($pg_no > 1) {
            $pageination .= "<button class='page-btn btn btn-success' onclick='fetch(" . ($pg_no - 1) . ")'>&lt;</button>";
            $pg = $pg_no;
        }
